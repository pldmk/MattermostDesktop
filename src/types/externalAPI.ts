...
export interface ExternalAPI {
    ...
    createListener(event: 'metrics-send', listener: (
        metricsMap: Map<string, { cpu?: number; memory?: number }>
    ) => void): () => void;

    // --- PTT события (Desktop → Renderer) ---
    createListener(event: 'PTT_EVENT_PRESSED', listener: () => void): () => void;
    createListener(event: 'PTT_EVENT_RELEASED', listener: () => void): () => void;
    createListener(event: 'PTT_EVENT_TOGGLE_MIC', listener: () => void): () => void;

    // --- Fallback для будущих каналов (держите последним перегрузом) ---
    createListener(event: string, listener: (...args: unknown[]) => void): () => void;
}
